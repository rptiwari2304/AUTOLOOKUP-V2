...
if file1 and file2:
    ...
    # ----------------- TAB ORDER (Compare first) -----------------
    tab1, tab2 = st.tabs(["🛠 Compare & Replace", "🚀 Auto Lookup"])

    # ----------------- TAB 1: COMPARE & REPLACE -----------------
    with tab1:
        ...
        if "mismatched1" in st.session_state and "mismatched2" in st.session_state:
            ...
            # Choice of replacement direction
            replace_direction = st.radio(
                "Replacement Direction:",
                ["Replace File2 → with File1 value", "Replace File1 → with File2 value"],
                key="replace_dir"
            )

            if st.button("➕ Add to Task List"):
                if selected_val1 and selected_val2:
                    st.session_state["task_list"].append((selected_val1, selected_val2, replace_direction))
            ...
            if st.session_state["task_list"]:
                st.write("### 📋 Task List")
                for i, (v1, v2, direction) in enumerate(st.session_state["task_list"]):
                ...
                if st.button("⚡ Execute Replacements"):
                    df2_clone = st.session_state["df2_clone"].copy()
                    for v1, v2, direction in st.session_state["task_list"]:
                        if direction == "Replace File2 → with File1 value":
                            df2_clone[col2_cr] = df2_clone[col2_cr].replace(v2, v1)
                        else:  # Replace File1 with File2
                            df2_clone[col2_cr] = df2_clone[col2_cr].replace(v1, v2)
